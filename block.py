...
def getGenesisBlock():
    return Block(0, '0', '1496518102.896031', "firstBlock",
                 '0q23nfa0se8fhPH234hnjldapjfasdfansdf23')
...
def generateNextBlock(blockData):
    ...
    print(previousBlock.currentHash)
    nextIndex = previousBlock.index + 1
    nextTimestamp = time.time()
    nextHash = calculateHash(nextIndex, previousBlock.currentHash,
                             nextTimestamp, blockData)
    return Block(nextIndex, previousBlock.currentHash,
                 nextTimestamp, blockData, nextHash)

# check to see if block properties match
...
def isSameBlock(block1, block2):
    ...
    # check to see if newly generate block has correct
    # properties based on comparison to prev block

def isValidNewBlock(newBlock, previousBlock):
    if previousBlock.index + 1 != newBlock.index:
        print('indices arent correct')
        return False
